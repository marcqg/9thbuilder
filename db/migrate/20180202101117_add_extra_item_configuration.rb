class AddExtraItemConfiguration < ActiveRecord::Migration[5.0]
  def change
    ...
    NinthAge::UnitOption.joins(:translations)
    					.where("name LIKE '%(one choice only)%'").update_all(:has_one_choise => true)
    NinthAge::UnitOption.joins(:translations)
    					.where("name LIKE '%(choose one)%'").update_all(:has_one_choise => true)
    ...
    NinthAge::ExtraItem.add_translation_fields! infos: :text
  end
end
