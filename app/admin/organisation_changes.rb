ActiveAdmin.register NinthAge::OrganisationChange do
  ...
  form do |f|
    f.inputs do
      ...
      f.input :type_target, as: :select, include_blank: false, collection: NinthAge::OrganisationChange.type_targets.keys.collect { |type_target| [I18n.t("unit.type_target.#{type_target}", default: type_target.titleize), type_target] }, include_blank: false, include_hidden: false
      f.input :number
    end
    f.actions
  end
